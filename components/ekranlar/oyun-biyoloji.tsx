...
import {
  turHazirla,
  type BiyolojiOyunSorusu,
  type BiyolojiSikki,
  type BiyolojiSorusu,
} from '@/lib/oyunlar/biyoloji'
import { ORTAK_HAVUZU } from '@/lib/oyunlar/ortak-havuzu'
import { SINIFLANDIRMA_HAVUZU } from '@/lib/oyunlar/siniflandirma-havuzu'
import {
  guncelSeri,
  rekorKirildiMi,
  turOzeti,
...
/**
 * Biyolojinin iki çoktan seçmeli oyunu — **tek ekran**.
 *
 * "Ortak Özellikler" ile "Canlıları Sınıflandır" arasındaki tek fark hangi
 * havuzdan çektikleri; ritim, şık düzeni, geri bildirim ve tur sonu birebir
 * aynı. İki ayrı dosya yazılsaydı dört yüz satır kopyalanır ve bir hata
 * düzeltildiğinde ötekinde kalırdı.
 *
 * Oyun kimliği prop olarak geliyor; havuz, zorluk anahtarı ve tur sonu metni
 * aşağıdaki tabloda ona bağlı.
 */

export type BiyolojiOyunId = 'ortak' | 'siniflandirma'
...
type GeriBildirim = { secilen: string | null; dogruMu: boolean; soru: BiyolojiSorusu }

/** Banka kayıtlarından soru havuzu; kayıt sorunun tamamını taşıyor. */
function bankaHavuzu(
  kayitlar: readonly BankaKaydi[],
  oyunId: BiyolojiOyunId,
): BiyolojiSorusu[] {
  const havuz: BiyolojiSorusu[] = []
  for (const kayit of kayitlar) {
    if (kayit.soru.oyun !== oyunId) continue
    havuz.push(kayit.soru.biyoloji)
  }
  return havuz
}

export function BiyolojiOyunuEkrani({
  oyunId,
  istatistik,
  sesAcik,
  bankaSorulari,
  ...
  const cevaplarRef = useRef<Cevap<BiyolojiSorusu>[]>([])
  ...
  const turBitir = useCallback(
    (verilenler: Cevap<BiyolojiSorusu>[]) => {
      ...
      onTurBitti(
        ...
        verilenler.map((cevap) => ({
          soru: biyolojidenBanka(oyunId, cevap.soru),
          dogruMu: cevap.dogruMu,
        })),
        Math.round((Date.now() - turBasladiRef.current) / 1000),
      )
    },
    [bankaTuru, gecerliMod, istatistik, onTurBitti, oyunId, sesAcik],
  )
  ...
  const cevapla = (sik: BiyolojiSikki) => {
    ...
    <>
      ...
      >
        {asama === 'bitti' && sonuc ? (
          <SonucGorunumu
            oyunId={oyunId}
            bolumBasligi={ayar.bolumBasligi}
            sonuc={sonuc}
            ...
            <>
              ...
              {geriBildirim && (
                <Bildirim
                  ...
                  aciklama={
                    geriBildirim.dogruMu ? undefined : `Doğrusu: ${geriBildirim.soru.dogru}`
                  }
                />
              )}
            </>
...
function SonucGorunumu({
  ...
  oyunId: BiyolojiOyunId
  bolumBasligi: string
  sonuc: { ozet: TurOzeti<BiyolojiSorusu>; yeniRekor: boolean }
  rekor: number
  bankaTuru: boolean
  mod: OyunModu
  elendi: Eleme
      ...
      {ozet.yanlislar.length > 0 && (
        <div className="flex flex-none flex-col gap-2">
          {gorunen.map((yanlis, sira) => (
            <YanlisKarti
              key={`${yanlis.soru}-${sira}`}
              oyunId={oyunId}
              soru={biyolojidenBanka(oyunId, yanlis)}
              bildir={bildir}
            >
