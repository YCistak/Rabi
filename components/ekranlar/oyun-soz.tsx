...
import { sozdenBanka, bankaKimligi, type BankaCevabi, type BankaKaydi } from '@/lib/oyunlar/banka'
import {
  elerMi,
  soruSuresi,
  turSirasi,
 ...
 * yere denk gelirdi.
 */
function sirayiKur(akis: SoruAkisi<SozSorusu>): SoruAkisi<SozOyunSorusu> {
...
export function SozOyunuEkrani({
  ...
  const [sorular, setSorular] = useState<SoruAkisi<SozOyunSorusu>>(tekAkis([]))
  ...
  */
  ...
  const turBaslat = useCallback(() => {
    ...
    setSorular(
      bankaTuru
        ...
        : sirayiKur(turSirasi(SOZ_HAVUZU, Math.random, TUR_SORU_SINIRI, {
              gorulenler,
              anahtar: (s) => bankaKimligi(sozdenBanka(s)),
            })),
    )
    zorluguSifirla()
    setSira(0)
  ...
  const turBitir = useCallback(
    (verilenler: Cevap<SozSorusu>[], yarim = false) => {
