...
import { ogedenBanka, bankaKimligi, type BankaCevabi, type BankaKaydi } from '@/lib/oyunlar/banka'
import {
  elerMi,
  soruSuresi,
  turSirasi,
 ...
 * yere denk gelirdi.
 */
function sirayiKur(akis: SoruAkisi<OgeSorusu>): SoruAkisi<OgeOyunSorusu> {
...
function bankaHavuzu(kayitlar: readonly BankaKaydi[]): OgeSorusu[] {
  ...
  for (const kayit of kayitlar) {
    ...
    havuz.push({
      ...
      // yanlışları. Alan tipin gereği doldurulmuş.
...
export function OgeOyunuEkrani({
  ...
  const [sorular, setSorular] = useState<SoruAkisi<OgeOyunSorusu>>(tekAkis([]))
  ...
  const turBaslat = useCallback(() => {
    ...
    setSorular(
      bankaTuru
        ...
        : sirayiKur(turSirasi(OGE_HAVUZU, Math.random, TUR_SORU_SINIRI, {
              gorulenler,
              anahtar: (s) => bankaKimligi(ogedenBanka(s)),
            })),
    )
    zorluguSifirla()
    setSira(0)
