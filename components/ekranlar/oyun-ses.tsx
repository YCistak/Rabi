...
import { sestenBanka, bankaKimligi, type BankaCevabi, type BankaKaydi } from '@/lib/oyunlar/banka'
import {
  elerMi,
  soruSuresi,
  turSirasi,
 ...
 * yere denk gelirdi.
 */
function sirayiKur(akis: SoruAkisi<SesSorusu>): SoruAkisi<SesOyunSorusu> {
...
function bankaHavuzu(kayitlar: readonly BankaKaydi[]): SesSorusu[] {
  ...
  for (const kayit of kayitlar) {
    ...
    havuz.push({
      ...
      // yanlışları, seviyesi ne olursa olsun tekrar edilecek. Alan tipin
      // gereği doldurulmuş durumda.
      zorluk: 'orta',
    })
  }
  return havuz
...
export function SesOyunuEkrani({
  ...
  const [sorular, setSorular] = useState<SoruAkisi<SesOyunSorusu>>(tekAkis([]))
  ...
  */
  ...
  const turBaslat = useCallback(() => {
    ...
    setSorular(
      bankaTuru
        ...
        : sirayiKur(turSirasi(SES_HAVUZU, Math.random, TUR_SORU_SINIRI, {
              gorulenler,
              anahtar: (s) => bankaKimligi(sestenBanka(s)),
            })),
    )
    zorluguSifirla()
    setSira(0)
  ...
  const turBitir = useCallback(
    (verilenler: Cevap<SesSorusu>[], yarim = false) => {
